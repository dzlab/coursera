...
- More computational power available x
...
- Either transcription is okay, but the inconsistency is problematic. x
...
- It is an iterative process, where during a later stage we might go back to an earlier stage. (That’s why we call it a cycle--it’s a circular process.) x
...
- Use a plan to ramp up to more users at a fixed rate: 1% in the first week, 2% in second week, 4% in third, and so on, so that the rollout can be well planned and managed. x
...
- Spammers trying to change the wording used in emails to get around your spam filter. x
...
- It is an iterative process, where you should expect to make multiple adjustments (such as metrics monitored using dashboards or percentage of traffic served) to work towards optimizing the system. x
